...
public class Particle {
	...
	public double getCollisionTime (Particle other) {
		System.out.println("getting collision time");
	 ...
	 * 
	 * @param width
	 * @return time to collide with a vertical wall
	 */
	 public double timeToCollideVerticalWall(double width) {
	        if (_vx > 0) return (width - _x - _radius) / _vx;
	        else if (_vx < 0) return (_x -_radius) / _vx;
	  ...
	  * 
	  * @param width
	  * @return time to collide with a horizontal wall
	  */
	  public double timeToCollideHorizontalWall(double width) {

	        if (_vy > 0) return (_y - _radius) / _vy;
	        else if (_vy < 0) return (width - _vy - _radius) / _vy;
	        return Double.POSITIVE_INFINITY;
	    }
	
	/**
	 * updates velocity of x
	 */
	...
	/**
	 * 
	 * @return Gets particle's last updates time
	 */
